...
class MusicXML {
  static parse(xml: string): Score {
    const conv = convert.xml2js(
      xml.replace(new RegExp("<!DOCTYPE(.*)>", "g"), ""),
      {
        compact: true,
      }
    ) as MusicXMLFile;
    ...
    scoreParts.forEach((part) => {
      ...
      if (!Array.isArray(scoreInstruments))
        scoreInstruments = [scoreInstruments];
      scoreInstruments.forEach((si) => {
